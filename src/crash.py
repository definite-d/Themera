...
def run_crash_window(error_message, sg):
    ...
    additional_info_layout = [
        ...
        [sg.Text('Please give any additional feedback on the issue.\n'
                 'Your system information will be included automatically.')],
        [sg.Multiline('-- No additional feedback --', k='additional_info', size=(50, 7))],
        [sg.HSep()],
...
def handle_crash(exception, sg):
    ...
    if issue_info:
        ...
        link = f'{LINK_NEW_GITHUB_ISSUE}?' + \
               parse.urlencode(args).replace("%5Cn", "%0D")
