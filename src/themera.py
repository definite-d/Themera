...
from constants import ALT, BATCH_ACTIONS, BORDER_UPPER_LIMIT, CTRL, CTRL_EVENT, GEAR_ICON, PENCIL_ICON, \
    WARNING_ICON, EXTERNAL_LINK_ICON, CONTRAST_THRESHOLD, LINK_GITHUB_REPO, LINK_DEVELOPER, LINK_NEW_GITHUB_ISSUE, \
    LINK_PYSIMPLEGUI_SITE, CONTRAST_THRESHOLD_MULTIPLIER, LINK_DOCS_DL, HELP_PATH, APP_ID
from crash import handle_crash
from filters import FILTERS, FILTER_MAPPING
from fonts import FONTS
from functions import alter_luminance, flatten_themedict, get_display_name, mini_preview_window_layout, random_color, \
    reskin_mini_preview_window, rint, unflatten_themedict, invert, check_if_color
...
if sg.running_windows():
    try:
        ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID(APP_ID)  # Adapted from PySimpleGUI source.
...
def menudef_to_shortcut_router_dict(menudef):
    return {tuple(subsection.split('(')[1].lstrip(' ')[0:-1].split('+')):
                subsection.split(' (')[0].replace('&', '')
            for section in menudef
            for subsection in section[1] if subsection.endswith(')')}
...
def get_proper_colorbox_textcolor(color):
    ...
    if abs(bg_color.get_luminance() - text_color.get_luminance()) <= CONTRAST_THRESHOLD:
        ...
        text_color.set_luminance(
            min(1, text_color.get_luminance() + (CONTRAST_THRESHOLD * CONTRAST_THRESHOLD_MULTIPLIER * sign))
            if sign == 1 else
            max(0, text_color.get_luminance() + (CONTRAST_THRESHOLD * CONTRAST_THRESHOLD_MULTIPLIER * sign))
        )
...
def themedict_entry(name, value, name_size=16, value_size=10):
    if check_if_color(value):
        text = get_proper_colorbox_textcolor(value) if SETTINGS['colorboxes'] else sg.theme_input_text_color()
        bg = value if SETTINGS['colorboxes'] else sg.theme_input_background_color()
        return \
            sg.Checkbox(f'{get_display_name(name)}', k=f'{name}_entryname', enable_events=True, pad=(0, 0),
                        size=(name_size, 1), tooltip=f'{get_display_name(name)}'), \
            sg.Text(WARNING_ICON, k=f'{name}_warning', font=FONTS['icon'], pad=(0, 0),
                    text_color=sg.theme_background_color()), \
            sg.Input(value, k=f'{name}_value', size=(value_size, 1), expand_x=True, metadata='is_color',
                     text_color=text, background_color=bg), \
            sg.Text(PENCIL_ICON, k=f'{name}_pickcolor', font=FONTS['icon'],
                    enable_events=True, pad=((0, 3), 0)),
    if type(value) == int:
        return \
            sg.Text(f'{get_display_name(name)}', k=f'{name}_entryname', pad=((23, 0), 0), size=(name_size, 1),
                    tooltip=f'{get_display_name(name)}'), \
            sg.Text(WARNING_ICON, k=f'{name}_warning', font=FONTS['icon'], pad=(0, 0),
                    text_color=sg.theme_background_color()), \
            sg.Spin([n for n in range(0, BORDER_UPPER_LIMIT)], value, k=f'{name}_value', expand_x=True,
                    size=(value_size, 1), metadata=type(value))
    if type(value) == float:
        return \
            sg.Text(f'{get_display_name(name)}', k=f'{name}_entryname', pad=((23, 0), 0), size=(name_size, 1),
                    tooltip=f'{get_display_name(name)}'), \
            sg.Text(WARNING_ICON, k=f'{name}_warning', font=FONTS['icon'], pad=(0, 0),
                    text_color=sg.theme_background_color()), \
            sg.Spin([n / 10 for n in range(0, BORDER_UPPER_LIMIT)], value, k=f'{name}_value', expand_x=True,
                    size=(value_size, 1), metadata=type(value))
    return \
        sg.Text(f'{get_display_name(name)}', k=f'{name}_entryname', pad=((23, 0), 0), size=(name_size, 1),
                tooltip=f'{get_display_name(name)}'), \
        sg.Text(WARNING_ICON, k=f'{name}_warning', font=FONTS['icon'], pad=(0, 0),
                text_color=sg.theme_background_color()), \
        sg.Input(str(value), k=f'{name}_value', expand_x=True, size=(value_size, 1), metadata=type(value))
...
def get_editor_window_layout(
        ...
        [
            sg.Column([
                [sg.Text(theme_name, k='theme_name', font=FONTS['theme_name']),
                 sg.Input(theme_name, k='theme_name_value', font=FONTS['theme_name'], visible=False, size=(15, 1)),
                 sg.Text(PENCIL_ICON, k='edit_theme_name', enable_events=True, visible=False, font=FONTS['icon'])]],
                pad=(0, 0), k='theme_name_container'),
            sg.Push(),
            sg.Text(GEAR_ICON, k='Settings', font=FONTS['icon'], enable_events=True),
        ],
    ]
    edit_frame_layout = [[sg.Checkbox('Select All', k='select_all', enable_events=True, pad=(0, 0))]]
    themedict_flat = flatten_themedict(themedict)
    max_name_size = rint(max([len(x) for x in [get_display_name(name) for name in themedict_flat]]) * 0.83)
    for name, value in themedict_flat.items():
        edit_frame_layout.append(themedict_entry(name, value, max_name_size))
    edit_frame_layout.append([sg.Column([[
        sg.DropDown(
            BATCH_ACTIONS,
            BATCH_ACTIONS[0],
            expand_x=True,
    ...
    ]], k='entry_action_container', pad=(0, 0), expand_x=True)])
    left_layout = [[sg.Frame('Theme Values', edit_frame_layout, k='edit_frame', element_justification='c',
                             expand_x=True, expand_y=True)]]
    right_layout = [
        [sg.Frame('Theme Preview and Generation',
                  [
                      [
                          sg.DropDown(FILTERS,
                                      FILTERS[0],
                      ...
                      [sg.Multiline(expand_y=True, expand_x=True, k='theme_code', disabled=True)],
                      [
                          sg.Button('Copy Theme Code', k='copy', pad=(4, 4)),
                          sg.Button('Full Preview', k='full_preview', pad=(4, 4)),
                          sg.Button('Apply Filter', k='apply_filter', pad=(4, 4), disabled=True),
                      ]
                  ], expand_y=True, expand_x=True, element_justification='c')
         ]
    ]
    return [
        [sg.Menu(menu_layout)],
        ...
        [sg.Column(left_layout, expand_y=True, expand_x=True, pad=((2, 1), 2)),
         sg.Column(right_layout, expand_y=True, expand_x=True, element_justification='c', pad=((1, 2), 2))]
    ]
...
class Editor:
    def __init__(self, theme_name, themedict=None):
        ...
        self.original_themedict = themedict.copy() if themedict else sg.LOOK_AND_FEEL_TABLE[theme_name].copy()
        self.values = {f'{k}_value': v for k, v in flatten_themedict(self.original_themedict).items()}
        self.invalids = set()
        self.previous_themedict = self.original_themedict.copy()
        self.preview_themedict = self.original_themedict.copy()
        self.window = Window(f'\'{self.theme_name}\' Theme | Themera v{__version__} Editor',
                                get_editor_window_layout(self.theme_name, self.original_themedict.copy(),
                                                         self.shortcut_router),
        ...
        self.window['edit_theme_name'].widget.bind('<Button-1>', lambda e: self.edit_theme_name_action())
        self.window['theme_name'].widget.bind('<Button-1>', lambda e: self.edit_theme_name_action())
        self.window['theme_name_container'].widget.bind('<Button-1>', lambda e: self.edit_theme_name_action())
    ...
    def color_picker(self, targets: List):
        if check_if_color(self.values[f'{targets[0]}_value']):
            ...
            title = f'{display_names[0]} Color' if len(display_names) == 1 else f'Color Picker ({", ".join(display_names)})'
            color = colorchooser.askcolor(parent=self.window.TKroot, color=self.values[f'{targets[0]}_value'], title=title)
            if color[1]:
    ...
    def theme_name_value_action(self):
        ...
        self.window.set_title(f'\'{self.theme_name}\' Theme | Themera v{__version__} Editor')
    ...
    def invalid_action(self, name):
        ...
        self.window[f'{name}_warning'].widget.configure(foreground=sg.theme_text_color())
    ...
    def valid_action(self, name):
        try:
            ...
            pass
        self.window[f'{name}_warning'].widget.configure(foreground=sg.theme_background_color())
    ...
    def validate_theme_values(self):
        for name in flatten_themedict(self.themedict):
            ...
            if self.window[key].metadata == 'is_color':
                ...
                try:
                    if value in (None, '', '-') or \
                            ((issubclass(value_type, int) or issubclass(value_type, float)) and value_type(value) < 0):
                        self.invalid_action(name)
                        continue
    ...
    def generate_theme_code(self, themedict):
        ...
        formatted_dict += '    }\n'
        return f'{SETTINGS["psg_alias"]}.theme_add_new(\n    \'{self.theme_name}\', ' \
    ...
    def update_quick_preview(self, themedict):
        ...
        self.preview_themedict = themedict.copy() if not current_filter else current_filter(themedict.copy())
    ...
    def full_preview(self):
        ...
        preview_layout = [
            ...
            [sg.Multiline(f'# This is the code responsible for this window\'s theme.\n'
                          f'\n{self.generate_theme_code(self.themedict)}', size=(58, 5))],
            ...
            [sg.Frame(title='Slider Preview', layout=[
              ...
              [sg.Slider(range=(0, 1000), size=(35, 10),
                         default_value=504, orientation='h')]
            ])],
    ...
    def read(self):
        ...
        while True:
            ...
            if len(self.invalids) == 0 and self.previous_themedict != self.themedict:  # No invalid values
            ...
            if event == 'View valid color names':
                ...
                viewer_layout = [
                    [sg.Text(text=(f'These are {len(colour.COLOR_NAME_TO_RGB)} valid color names.'))],
                    [sg.Text('Ranked from darkest to lightest.')],
                    [sg.Column(layout=color_names, size=(270, 200), scrollable=True, vertical_scroll_only=True,
                               background_color=sg.theme_input_background_color(), element_justification='center')],
                    [sg.Text('Clicking a color\'s button copies the color.')],
                    [sg.Button('Exit')]
                ]
                viewer = Window('Valid Color Name List', layout=viewer_layout,
                                   element_justification='center', modal=False)
                while True:
                    viewer_e = viewer()[0]
                ...
                if isfile(_path):
                    ...
                    not_found_window = Window(
                        ...
                        [
                            ...
                            [sg.Push(), sg.Button('Download the Docs', k='dl'), sg.Button('Cancel')],
                        ],
                        modal=True,
                    )
            ...
            if event == 'execute_entry_action':
                ...
                if value == 'Interpolate':
                    ...
                    colors = [
                        colour.hsl2hex(c)
                        for c in colour.color_scale(colour.web2hsl(begin), colour.web2hsl(end), len(selected))]
                    self.window.fill(dict(zip([f'{k}_value' for k in selected], colors)))
                if value == 'Shuffle':
            ...
            if event.startswith('select_all') or event.endswith('entryname'):
                ...
                if len(selected) >= 2:  # If there's 2 or more selected, enable the batch options.
            ...
            if event.startswith('Revert to Beginning') and (self.themedict != self.original_themedict):
            ...
            if event == 'copy':
                ...
                self.copy_id = self.window['copy'].widget.after(2000, lambda: self.window['copy']('Copy Theme Code'))

            if event.startswith('Return to Launcher'):
                if sg.PopupYesNo('This action will close the editor. Continue?', title='Are you sure?') == 'Yes':
