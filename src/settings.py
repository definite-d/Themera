...
class Settings:
    ...
    def edit(self, sg, editor):
        ...
        layout = [
            [sg.Frame('Import Alias', [
                [
                    sg.Text('import PySimpleGUI as '),
                    sg.Input(self['psg_alias'], k='psg_alias_setting', size=(10, 1)),
                ]
            ],),
             sg.Frame('UI Theme', [
                [sg.DropDown(THEMES, self['theme'], k='theme_setting', enable_events=True, readonly=True)],
            ],),
             sg.Frame('Colorboxes', [
                [
                    sg.Checkbox('Enabled', default=self['colorboxes'], k='colorboxes_setting'),
                ]
            ], )],
            [sg.Frame('Filter Indexes', [
                [
                    sg.Text(f'Images', size=(15, 1)),
                    sg.Input(str(self['image_index']), k='image_index_setting', expand_x=True)
                ],
                [
                    sg.Text('Auto Contrast (Dark)', size=(15, 1)),
                    sg.Input(str(self['autocontrast_index_dark']), k='autocontrast_index_dark_setting', expand_x=True)
                ],
                [
                    sg.Text('Auto Contrast (Light)', size=(15, 1)),
                    sg.Input(str(self['autocontrast_index_light']), k='autocontrast_index_light_setting', expand_x=True)
                ],
                [
                    sg.Text('Dark Mode', size=(15, 1)),
                    sg.Input(str(self['dark_mode_index']), k='dark_mode_index_setting', expand_x=True)
                ],
                [
                    sg.Text('Light Mode', size=(15, 1)),
                    sg.Input(str(self['light_mode_index']), k='light_mode_index_setting', expand_x=True)
                ],
            ], expand_x=True)],
            [sg.HSep()],
            [sg.Push(), sg.Button('Save'), sg.Button('Apply'), sg.Button('Cancel'), ]
        ]
        ...
        while True:
            ...
            if e == 'theme_setting':
                reskin(settings_window, v['theme_setting'], sg.theme,
                       sg.LOOK_AND_FEEL_TABLE, target_element_keys=['theme_setting'], reskin_background=False)
                settings_window['theme_setting'].ParentRowFrame.configure(background=sg.theme_background_color())

            if e in ('Apply', 'Save'):

                if v['theme_setting'] != self['theme']:
                    reskin(
                        settings_window, v['theme_setting'],
                        sg.theme,
                ...
                for window in settings_window.open_windows:
                    if v['theme_setting'] != self['theme']:
                        reskin(window, v['theme_setting'], sg.theme, sg.LOOK_AND_FEEL_TABLE, True, honor_previous=False)
                        sg.set_options(border_width=0)

                    if window.editor_object:
                        if v['theme_setting'] != self['theme']:
                            window.editor_object.update_quick_preview(editor.preview_themedict)
                        ...
                        for k in window.key_dict.keys():
                            ...
                            if v['colorboxes_setting'] != self['colorboxes'] or v['theme_setting'] != self['theme']:
                                if str(k).endswith('value') and window[k].metadata == 'is_color':
                                    if v['colorboxes_setting']:
                                        window.editor_object._update_color_value(k, window[k].widget.get())
                                    else:
