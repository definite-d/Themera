...
Here lies a multitude of themes generated with Themera, ready for use. Simply copy the definition of the required theme and paste for use in your own PySimpleGUI project. If you use the themes, a little credit would be nice, but is not required.
'''
...
# Generated using Themera v2.6.34.
import PySimpleGUI as sg  # Please change 'sg' to your liking.
...
for theme_name in sample_themes:
	sg.theme(theme_name)
	bg_c = sg.theme_background_color()
	txt_c = sg.theme_text_color()
	in_txt_c = sg.theme_input_text_color()
	in_c = sg.theme_input_background_color()
	scr_c = sg.theme_slider_color()
	bt_txt_c = sg.theme_button_color()[0]
	bt_c = sg.theme_button_color()[1]
	prog_c = sg.theme_progress_bar_color()[0]
	prog_bg_c = sg.theme_progress_bar_color()[1]
	viewer_panel_layout.append(
		[sg.Frame(f'{theme_name}', [
			[
				sg.Button('', tooltip=bg_c, size=(3, 1), pad=(0, 2), button_color=(bg_c, bg_c)),
				sg.Button('', tooltip=txt_c, size=(3, 1), pad=(0, 2), button_color=(txt_c, txt_c)),
				sg.Button('', tooltip=in_txt_c, size=(3, 1), pad=(0, 2), button_color=(in_txt_c, in_txt_c)),
				sg.Button('', tooltip=in_c, size=(3, 1), pad=(0, 2), button_color=(in_c, in_c)),
				sg.Button('', tooltip=scr_c, size=(3, 1), pad=(0, 2), button_color=(scr_c, scr_c)),
				sg.Button('', tooltip=bt_txt_c, size=(3, 1), pad=(0, 2), button_color=(bt_txt_c, bt_txt_c)),
				sg.Button('', tooltip=bt_c, size=(3, 1), pad=(0, 2), button_color=(bt_c, bt_c)),
				sg.Button('', tooltip=prog_c, size=(3, 1), pad=(0, 2), button_color=(prog_c, prog_c)),
				sg.Button('', tooltip=prog_bg_c, size=(3, 1), pad=(0, 2), button_color=(prog_bg_c, prog_bg_c)),
				],
			[sg.Button('Copy Theme Code', key=f'copy_{theme_name}')]
		], element_justification='center')]
	)

sg.theme('SystemDefault')
main_window_layout = [
	[sg.Text('Themera Sample Themes', font=('Helvetica', 24))],
	[sg.Text('These are some themes generated with Themera.')],
	[sg.Text('Feel free to use them in your projects.')],
	[sg.Column(viewer_panel_layout, scrollable=True, vertical_scroll_only=True, size=(340, 400), element_justification='center')],
	[sg.Text('Clicking to copy them copies the dictionary of color values.')],
	[sg.Text('You\'ll have to use the theme_add_new() function yourself.')],
]

main_window = sg.Window('Sample Themes Previewer', main_window_layout, element_justification='center')

while True:
	main_window_events, main_window_values = main_window.Read()

	if main_window_events in (None, 'Exit'):
		main_window.Close()
		break

	if 'copy' in main_window_events:
		theme_name = main_window_events.split('_', maxsplit=1)[1]
		copy(str(sg.LOOK_AND_FEEL_TABLE[theme_name]))
		sg.Popup(f'The theme dictionary of {theme_name} has been copied.', title='Copied Theme Code Successfully.')